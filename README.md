...
Everything degrades gracefully: no media, no LibreHardwareMonitor, no VS Code —
those widgets simply hide, nothing breaks.
...
- **In the browser:** open **`http://<pc-ip>:7777/config`** — a settings page for
  the weather city, game list, Pomodoro lengths, and port. It tells you which
  changes apply live vs. need a restart.
...
(weather), a server-side Pomodoro timer, and a POST endpoint fed by the VS Code
extension. A context engine picks the scene by priority (focus > music > game >
dev > idle) with hysteresis, a manual override, and a hard lock. Target idle
cost: <1% CPU, <100 MB RAM.
...
desko/                # server, state, context engine, focus timer, collectors, demo
  collectors/         #   media, lyrics, sysstats, weather, vscode
